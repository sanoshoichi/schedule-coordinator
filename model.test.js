import test from'node:test';import assert from'node:assert/strict';import{validDate,normalizeName,parseDates,buildSlots,nextMark,normalizeHourRange,validateAvailability}from'./model.js';
...
test('時間帯は選択順に関係なく1時間単位の範囲になる',()=>{assert.deepEqual(normalizeHourRange(18,9),{start:9,end:18});assert.deepEqual(normalizeHourRange(-2,27),{start:0,end:23})});
