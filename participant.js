...
try{schedule=await getSchedule(slug);if(Array.isArray(schedule))schedule=schedule[0];if(!schedule)throw new Error('日程が見つかりません');render();message('')}catch(e){message(`読み込めませんでした: ${e.message}`,true);$('scheduleTitle').textContent='日程を表示できません'}
